...
def adding_noise(embeddings, mean, std):
    noise = torch.randn(embeddings.shape) * std + mean
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mean_of_noise', default=0, type=int, help='the mean of the noise')
    parser.add_argument('--std_of_noise', default=0.5, type=int, help='the std of the noise')
    ...
    with torch.no_grad():
        if args.proto_patch != 1:
            for (data, labels) in tqdm(proto_data_loader, desc='Prototypical feature extraction'):
                ...
                # adding noise
                target_embeddings = adding_noise(target_embeddings, mean=args.mean_of_noise, std=args.std_of_noise)
    ...
    for epoch in range(epoch_start, args.epochs+1):
        ...
        if epoch % args.test_freq == 0:
            ...
            if test_acc >= best_acc:
                best_acc = test_acc
                torch.save({'state_dict': surrogate_model_new.state_dict()}, args.model_results_dir + '/our_model_noise_0.5_seed{}_'.format(args.seed)+ args.query_dataset + '.pth')
