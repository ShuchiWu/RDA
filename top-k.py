...
def top_k(target_embeddings, k=10):
    zeroed_tensor = torch.zeros_like(target_embeddings)
    top_values, top_indices = torch.topk(target_embeddings, k=k, sorted=True)

    for sample, dimension in enumerate(zeroed_tensor):
        zeroed_tensor[sample][top_indices[sample]] = top_values[sample]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--top_k', default=5, type=int, help='top-k to retain')
    ...
    # else:
    #     _ = test(target_model, memory_loader, test_loader, 0, args)  # CIFAR10 Pre-trained: CIFAR10: 88.31%  STL10: 71.59%  ImageNet Pre-trained: CIFAR10:81.21% STL10: 71.99 (112*112)
    ...
    with torch.no_grad():
        if args.proto_patch != 1:
            for (data, labels) in tqdm(proto_data_loader, desc='Prototypical feature extraction'):
                ...
                # zeroed_tensor = torch.zeros_like(target_embeddings)
                # top_values, top_indices = torch.topk(target_embeddings, k=10, sorted=True)
                #
                # for sample, dimension in enumerate(zeroed_tensor):
                #     zeroed_tensor[sample][top_indices[sample]] = top_values[sample]
                top_k_embeddings = top_k(target_embeddings, k=args.top_k)

                target_embedding_avg = torch.squeeze(chunk_avg(top_k_embeddings, args.proto_patch))
            ...
            for (data, labels) in tqdm(proto_data_loader, desc='Prototypical feature extraction'):
                ...
                # for sample, dimension in enumerate(zeroed_tensor):
                #     zeroed_tensor[sample][top_indices[sample]] = top_values[sample]
                top_k_embeddings = top_k(target_embeddings)

                target_embedding_avg = torch.squeeze(top_k_embeddings, k=args.top_k)
                for index, label in enumerate(labels):
                    prototypes_bank[int(label)] = target_embedding_avg[index]
    end_time = time.time()
    ...
    for epoch in range(epoch_start, args.epochs+1):
        ...
        if epoch % args.test_freq == 0:
            ...
            if test_acc >= best_acc:
                best_acc = test_acc
                torch.save({'state_dict': surrogate_model_new.state_dict()}, args.model_results_dir + '/our_model_top_k_5_seed{}_'.format(args.seed)+ args.query_dataset + '.pth')
