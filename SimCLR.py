...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_dir', default='results', type=str, metavar='PATH', help='path to save the results (default: none)')
    ...
    for epoch in range(epoch_start, args.epochs + 1):
        ...
        data_frame.to_csv(args.results_dir + '/simclr_cifar10_mobilenet_v2_log.csv', index_label='epoch')
        # Save model
        # torch.save({'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer' : optimizer.state_dict(),}, args.results_dir + '/model_last.pth')

        if epoch % args.epochs == 0:
            torch.save({'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, args.results_dir + '/cifar10_model_mobilenet_v2_' + str(epoch) + '.pth')
