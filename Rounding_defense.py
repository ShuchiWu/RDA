...
if __name__ == '__main__':
    ...
    parser.add_argument('--decimals', default=1, type=int, help='precision of the rounding')
    ...
    with torch.no_grad():
        if args.proto_patch != 1:
            for (data, labels) in tqdm(proto_data_loader, desc='Prototypical feature extraction'):
                ...
                target_embeddings = torch.round(target_embeddings, decimals=args.decimals)
    ...
    for epoch in range(epoch_start, args.epochs+1):
        ...
        if epoch % args.test_freq == 0:
            ...
            if test_acc >= best_acc:
                best_acc = test_acc
                torch.save({'state_dict': surrogate_model_new.state_dict()}, args.model_results_dir + '/our_model_round_1_seed{}_'.format(args.seed)+ args.query_dataset + '.pth')
