...
from datasets import get_query_dataset, get_pretraining_dataset, get_proto_dataset, get_downstream_test_data, get_pair_query_stolenencoder, get_pair_query_consteal
...
class ConStealLoss(nn.Module):
    def __init__(self, temperature=0.07, batch_size=100):
        super(ConStealLoss, self).__init__()
        self.temperature = temperature
        self.batch_size = batch_size

    def forward(self, x, y):
        sim_matrix_1 = torch.exp(torch.mm(x, y.t()) / self.temperature)
        sim_matrix_2 = torch.exp(torch.mm(y, y.t()) / self.temperature)
...
def steal_train(args, query_data_loader, surrogate_model, target_model, steal_optimizer, loss_func, epoch):
    ...
    for batch, (img_1, img_2, label) in tqdm(enumerate(query_data_loader), desc='Training process of epoch [{}/{}]'.format(epoch, args.epochs)):
        ...
        img_2 = img_2.cuda()
        with torch.no_grad():
            surrogate_embeddings_1 = F.normalize(target_model(img_1), dim=-1)
        surrogate_embeddings_2 = F.normalize(surrogate_model(img_2), dim=-1)

        loss = 0
        if args.loss_function == 'con-steal':
            loss += loss_function(torch.squeeze(surrogate_embeddings_1), torch.squeeze(surrogate_embeddings_2))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss_function', default='con-steal', type=str, help='loss function used for attacker', choices=['mse', 'cos_similarity', 'cross_entropy', 'ours'])
    ...
    parser.add_argument('--knn-k', default=100, type=int, help='k in kNN monitor')
    ...
    query_dataset = get_pair_query_consteal(args)
    ...
    for epoch in range(epoch_start, args.epochs+1):
        ...
        surrogate_model_new = steal_train(args, query_data_loader, surrogate_model, target_model, steal_optimizer, criterion, epoch)

        if epoch == 10:
            end_time = time.time()
            print('1 training epoch takes {:.2f} seconds (average)'.format((end_time - begin_time) / 10), flush=True)
        ...
        if epoch % args.test_freq == 0:
            ...
            data_frame.to_csv(args.log_results_dir + '/ContSteal_log_seed{}.csv'.format(args.seed), index_label='epoch')
            if test_acc >= best_acc:
                best_acc = test_acc
                # torch.save({'state_dict': surrogate_model_new.state_dict()}, args.model_results_dir + '/ContSteal_seed{}_'.format(args.seed)+ args.query_dataset + '.pth')
