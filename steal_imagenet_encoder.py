...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretraining_dataset', default='imagenet', type=str, help='dataset the victim uses for pretraining')
    ...
    parser.add_argument('--query_data_num', default=40000, type=int, help='how much query data we use')
    parser.add_argument('--target_downstream_dataset', default='stl10', type=str, help='target downstream dataset')
    parser.add_argument('--target_check_point', default='results/imagenet_resnet50.pth', type=str, help='path to the check point of the target model')
    ...
    for epoch in range(epoch_start, args.epochs+1):
        ...
        if epoch % args.test_freq == 0:
            ...
            data_frame.to_csv(args.log_results_dir + '/steal_imagenet_encoder_log_seed{}.csv'.format(args.query_patch, args.seed), index_label='epoch')

            if test_acc >= best_acc:
                best_acc = test_acc
                torch.save({'state_dict': surrogate_model_new.state_dict()}, args.model_results_dir + '/our_model_steal_imagenet_encoder_proto_patch_5_resnet18_100epochs_seed{}_'.format(args.seed)+ args.query_dataset + '.pth')
