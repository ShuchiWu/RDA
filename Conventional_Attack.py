...
from datasets import get_query_dataset, get_pretraining_dataset, get_proto_dataset, get_pair_query_conventional, get_downstream_test_data
...
def steal_train(args, query_data_loader, prototype, surrogate_model, steal_optimizer, loss_func, epoch):
    ...
    for batch, (img_1, img_2, label) in tqdm(enumerate(query_data_loader), desc='Training process of epoch [{}/{}]'.format(epoch, args.epochs)):
        img_2 = img_2.cuda()
        surrogate_embeddings = F.normalize(surrogate_model(img_2), dim=-1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--query_dataset', default='imagenet', type=str, help='dataset used for querying the victim')
    ...
    parser.add_argument('--loss_function', default='mse', type=str, help='loss function used for attacker', choices=['mse', 'cos_similarity', 'cross_entropy'])
    ...
    query_dataset = get_pair_query_conventional(args)
    memory_data, test_data = get_downstream_test_data(args)

    proto_data_loader = DataLoader(query_dataset, batch_size=args.proto_batch_size, shuffle=False,
                                   num_workers=12,
                                   pin_memory=True)
    ...
    #     _ = test(target_model.visual, memory_loader, test_loader, 0, args) # CIFAR10 Pre-trained: CIFAR10: 88.31%  STL10: 71.59%  GTSRB: 61.3%  SVHN: 40.77%
    # else:
    #     _ = test(target_model, memory_loader, test_loader, 0, args)
    ...
    with torch.no_grad():
        for (img_1, img_2, labels) in tqdm(proto_data_loader, desc='Prototypical feature extraction'):
            img_1 = img_1.cuda()
            target_embeddings = F.normalize(target_model(img_1), dim=-1).detach().cpu()
            for index, label in enumerate(labels):
                prototypes_bank[int(label)] = target_embeddings[index]
    end_time = time.time()
    ...
    for epoch in range(epoch_start, args.epochs + 1):
        ...
        if epoch % args.test_freq == 0:
            ...
            data_frame.to_csv(args.log_results_dir + '/Conventional_log_seed{}.csv'.format(args.seed), index_label='epoch')
            if test_acc >= best_acc:
                best_acc = test_acc
                # torch.save({'state_dict': surrogate_model_new.state_dict()}, args.model_results_dir + '/Conventional_seed{}'.format(args.seed) + args.query_dataset + '.pth')
